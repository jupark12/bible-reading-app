...
export const BibleReader: React.FC<BibleReaderProps> = ({
  ...
  const handleSelectionChange = (selectedBook: string, selectedChapter: string) => {
  ...
  const handleVerseClick = (verse: Verse) => {
    ...
    const verseExistsIndex = favorites.findIndex((fav) => fav.verse_id === verse.id);
    ...
    if (verseExistsIndex !== -1) {
      ...
      if (favorites.length >= MAX_FAVORITES) {
        ...
        toast.error(`Maximum of ${MAX_FAVORITES} favorite verses reached for today's devotional.`);
      } else {
      ...
      <ResizablePanel defaultSize={50} minSize={25}>
        <div className="flex h-full flex-col p-4">
          ...
          <div className="flex-grow overflow-y-auto">
            ...
            {loading ? (
              <div className="flex h-full items-center justify-center">Loading verses...</div>
            ) : chunks.length === 0 ? (
              ...
              chunks.map((chunk, chunkIndex) => (
                <p key={chunkIndex} className="verse-paragraph mb-2 leading-relaxed">
                  {' '}
                  {/* Add margin-bottom */}
